...
负责把生成的计划投递给人工审核渠道，例如 IM 机器人或其他审核入口。
...
V1 建议使用 SQLite 作为本地持久化层。

原因如下：

- V1 只有一个服务进程
- 执行并发被刻意限制得很低
- 当前阶段更重视本地可靠恢复，而不是水平扩展
...
如果 Hermes 与本编排程序部署在同一台机器上，应优先使用本地地址或内网地址通信，而不是额外暴露公网端口。
